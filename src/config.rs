...
pub const DEFAULT_CONFIG_JSON: &str = r#"{
  ...
  "routing": {
    ...
    "builder": { "tool": "codex", "effort": "medium" },
...
const CONFIG_JSON_CLAUDE_ONLY: &str = r#"{
  ...
  "routing": {
    "manager": { "tool": "claude", "model": "sonnet", "effort": "medium" },
    "architect": { "tool": "claude", "model": "opus", "effort": "xhigh" },
    "builder": { "tool": "claude", "model": "sonnet", "effort": "medium" },
    "customer": { "tool": "claude", "model": "haiku", "effort": "low" },
    "resolver": { "tool": "claude", "model": "sonnet", "effort": "medium" }
  },
  "defaults": { "role": "builder" }
...
const CONFIG_JSON_CODEX_ONLY: &str = r#"{
  ...
  "routing": {
    "manager": { "tool": "codex", "effort": "medium" },
    "architect": { "tool": "codex", "effort": "xhigh" },
    "builder": { "tool": "codex", "effort": "medium" },
    "customer": { "tool": "codex", "effort": "low" },
    "resolver": { "tool": "codex", "effort": "medium" }
  },
  "defaults": { "role": "builder" }
}
"#;
    ...
    fn all_default_configs_share_the_same_caps_and_roles() {
        ...
        for json in variants {
            ...
            for role in ROLES {
                ...
                // claude model aliases (opus/sonnet/haiku) are stable — pin
                // them; codex slugs churn (gpt-5 → gpt-5.5 → …), so codex
                // roles stay unpinned and use the CLI's own default.
                match cfg.role_field(role, "tool").as_deref() {
                    Some("claude") => assert!(
                        cfg.role_field(role, "model").is_some(),
                        "missing {role} model"
                    ),
                    _ => assert!(
                        cfg.role_field(role, "model").is_none(),
                        "{role} pins a codex model (slugs churn; leave unpinned)"
                    ),
                }
            }
        }
